...
@export_range(-4096, 4096)
var drag_z_index: int = 1000
...
func _ready():
	...
	assert(
		o != null,
		"Draggable node '%s' must be owned by an Area2D node" % name
	)
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if not ProjectSettings.has_setting("input/" + drag_input_name):
		warnings.append(
			"Action " + str(drag_input_name) + " could not be found in the InputMap"
		)
