...
enum SNAP_STYLE {
	...
	SNAP_CENTER,
	## Draggable snaps to marker children of the DropZone.
	## If there are no marker children, defaults to NO_SNAP
	SNAP_MARKERS
}
...
func _ready():
	...
	assert(
		o != null,
		"DropZone node '%s' must be owned by an Area2D node" % name
	)
